...
## TASK-180 전체 이력 DB 재구축 (다른 모든 항목의 선행 조건)
...
## TASK-181 재구성 모델을 2008년 실물 QLD 로 검증

QLD 는 2006-06-21 상장이라 2008년 폭락(QQQ −53.6%, QLD −84.0%)을 실물로 겪었다.
재구성 모델을 같은 구간에 돌려 **폭락기에 한정한** 추적오차를 잰다. 기존
측정(`synthetic.py`)은 전체 수명 기준이라 잔잔한 기간이 지배한다.

§2.11 의 −62.4% 를 얼마나 믿을 수 있는지가 여기서 결정된다.
